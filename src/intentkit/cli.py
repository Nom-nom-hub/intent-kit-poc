...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_path(init)
    init.add_argument("--project-name", help="Human-readable project name. Defaults to the directory name.")
    ...
    capture.add_argument("--description", required=True, help="Why the outcome matters and what should change.")
    capture.add_argument("--success-measure", action="append", default=[], help="Measurable success criterion. Repeatable.")
    capture.add_argument("--assumption", action="append", default=[], help="Key assumption. Repeatable.")
    capture.set_defaults(handler=handle_capture)

    shape = subparsers.add_parser("shape", help="Add a requirement, decision, and proof obligation for an outcome.")
    add_path(shape)
    ...
    shape.add_argument("--risk", choices=["R0", "R1", "R2", "R3"], default="R1", help="Risk class used to calibrate proof.")
...
def add_path(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--path", default=".", type=Path, help="Project directory. Defaults to the current directory.")
...
def handle_shape(args: argparse.Namespace) -> int:
    ...
    if args.proof_title or args.proof_description:
        if not args.proof_title or not args.proof_description:
            raise ValueError("A proof obligation requires both --proof-title and --proof-description.")
...
def handle_prove(args: argparse.Namespace) -> int:
    ...
    evidence_status = NodeStatus.VERIFIED if args.result == "pass" else NodeStatus.FAILED if args.result == "fail" else NodeStatus.ACTIVE
    ...
    graph.set_status(
        obligation.id,
        NodeStatus.VERIFIED if args.result == "pass" else NodeStatus.FAILED if args.result == "fail" else NodeStatus.ACTIVE,
    )
    store.save(graph)
    MarkdownRenderer(args.path).render(graph)
...
def handle_status(args: argparse.Namespace) -> int:
    ...
    for node in graph.nodes.values():
        counts[node.type] += 1
    obligations = [node for node in graph.nodes.values() if node.type == NodeType.PROOF_OBLIGATION.value]
