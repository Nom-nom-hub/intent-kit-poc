...
def test_cli_end_to_end_workflow(tmp_path: Path, capsys) -> None:
    ...
    assert main([
        "capture", "Prevent duplicate orders", "--path", root,
        "--description", "Retries must not create duplicate orders.",
        "--success-measure", "A repeated confirmation returns one order.",
    ]) == 0
    assert main([
        "shape", "Use idempotency keys", "--path", root,
        "--description", "Every confirmation request must carry an idempotency key.",
        "--outcome", "OUT-001", "--risk", "R3",
        "--decision-title", "Use provider idempotency keys",
        "--rationale", "Provider-backed idempotency is safest.",
    ...
    assert main([
        "prove", "PRF-001", "Payment contract test", "--path", root,
        "--description", "The retry contract passed against the payment sandbox.",
        "--source", "tests/test_payment_contract.py", "--result", "pass",
    ]) == 0
...
def test_cli_runs_a_checker_backed_proof(tmp_path: Path, capsys) -> None:
    ...
    assert main([
        "capture", "Ship a proof-backed release", "--path", root,
        "--description", "The release must include the required marker.",
    ]) == 0
    assert main([
        "shape", "Require the release marker", "--path", root,
        "--description", "The release marker must be present in the project.",
        "--outcome", "OUT-001",
        "--proof-title", "Check the release marker",
        "--proof-description", "release-marker.txt must exist.",
    ...
    assert main([
        "check", "PRF-001", "--path", root,
        "--checker", "local.file-exists",
        "--config", '{"path":"release-marker.txt"}',
    ]) == 0
