...
def test_renderer_creates_linked_markdown_views(tmp_path: Path) -> None:
    ...
    outcome = graph.add_node(
        NodeType.OUTCOME, "Prevent duplicate orders", "Checkout retries must be safe."
    )
    requirement = graph.add_node(
        NodeType.REQUIREMENT, "Use idempotency keys", "The retry must not create a duplicate order."
    )
    ...
    proof = graph.add_node(
        ...
        "Prove a retry is safe",
        "A repeated confirmation must return one order.",
    )
